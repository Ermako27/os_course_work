...
static asmlinkage long new_sys_execve(const char __user * filename,
	...
	char *uid_flag = "UID:";
	char *pid_flag = "PID:";
	char *rss_usage_flag = "RSS usage:";
	char *vm_mem_flag = "VM usage:";
	char *prio_flag = "Prio:";
	char *nice_flag = "Nice:";
	char *utime_flag = "Utime:";
	char *stime_flag = "Stime:";
	...
	if (NULL != exec_str) {
		...
		snprintf(exec_str, exec_line_size, "%s %s %llu", exec_str, rss_usage_flag, current->acct_rss_mem1);
		snprintf(exec_str, exec_line_size, "%s %s %llu", exec_str, vm_mem_flag, current->acct_vm_mem1);

		snprintf(exec_str, exec_line_size, "%s %s %d", exec_str, prio_flag, current->prio);
		snprintf(exec_str, exec_line_size, "%s %s %d", exec_str, nice_flag, task_nice(current));

		snprintf(exec_str, exec_line_size, "%s %s %llu", exec_str, utime_flag, current->utime);
		snprintf(exec_str, exec_line_size, "%s %s %llu", exec_str, stime_flag, current->stime);
