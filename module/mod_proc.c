...
MODULE_LICENSE( "GPL" );
MODULE_AUTHOR( "Melnikov Dmitry" );

static char *msg_tree;
static char *msg_mem;
static char *msg_files;

ssize_t read_proc_tree(struct file *filp,char *buf,ssize_t count,loff_t *offp ) 
{
...
ssize_t write_proc_tree(struct file *filp,const char *buf,ssize_t count,loff_t *offp)
{	
...
ssize_t read_proc_mem(struct file *filp,char *buf,ssize_t count,loff_t *offp ) 
{
	char msg2[] = "Pass id of process";
	char *buf_msg = vmalloc(10000 * sizeof(char));
...
ssize_t write_proc_mem(struct file *filp,const char *buf,ssize_t count,loff_t *offp)
{
	ssize_t procfs_buf_size = count;
	if (procfs_buf_size > MAX_WRITE_BUF_SIZE)
	{
...
ssize_t read_proc_files(struct file *filp,char *buf,ssize_t count,loff_t *offp ) 
{
	char msg2[] = "Pass id of process";
	char *buf_msg = vmalloc(10000 * sizeof(char));	
...
ssize_t write_proc_files(struct file *filp,const char *buf,ssize_t count,loff_t *offp)
{
	ssize_t procfs_buf_size = count;
	if (procfs_buf_size > MAX_WRITE_BUF_SIZE)
	{
...
static int  __init proc_init( void )
{
	...
	static const struct file_operations proc_file_fops_tree = {
		.owner = THIS_MODULE,
	 	.write = write_proc_tree,
	 	.read = read_proc_tree,
	};

	static const struct file_operations proc_file_fops_mem = {
		.owner = THIS_MODULE,
	 	.write = write_proc_mem,
	 	.read = read_proc_mem,
	};

	static const struct file_operations proc_file_fops_files = {
		.owner = THIS_MODULE,
	 	.write = write_proc_files,
	 	.read = read_proc_files,
	};
